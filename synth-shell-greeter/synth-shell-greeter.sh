...
# shellcheck disable=SC2155,SC2059,SC2004,SC1090
...
printTwoElementsSideBySide()
{
	...
	if [ "$e_1_cols" -gt 0 -a "$e_2_cols" -gt 0 ]; then
...
assert_not_empty()
{
	...
	local variable=$1
	if [ -z "$variable" ]; then 
		echo "Assertion failed, variable empty."
		return $assert_failed
	else
		return $ok
	fi
...
printStatusInfo()
{
	...
	statusSwitch()
	{
		...
		##	NAME            FUNCTION               AS %
			...
			*)              printInfoLine "Unknown $1" "Check your config";;
		esac
	}
	## ASSEMBLE INFO PANE
	local status_info=""
